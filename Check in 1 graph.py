...
########### HERE!!! ##############
    ...
    start = min(xdata)
    stop = max(xdata)    
    xs = np.arange(start,stop,(stop-start)/1000000) 
    ...
    ax1.plot(xs, curve, label="Flow velocity (µm/s) vs. Height of syringe from chip (cm)", color="blue")
    ...
    ax1.set_xlabel("Height of syringe from chip (cm)")
    ax1.set_ylabel("Flow velocity (µm/s)")
    ax1.set_title("Flow velocity (µm/s) vs. Height of syringe from chip (cm)")
...
########### HERE!!! ##############    
    ...
    for i in range(len(yerror)):
        chi_squared += ((residual[i])**2 / (yerror[i])**2)
    print('Chi squared:', chi_squared)

    degrees_of_freedom = len(ydata) - 2
